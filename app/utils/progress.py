...
Содержит абстрактный интерфейс для реализации различных стратегий
отслеживания прогресса, а также реализацию для Streamlit.
...
class AbstractProgress(ABC, Generic[T]):
    ...
    Определяет метод wrap для обёртывания итерируемого объекта с целью
...
class Progress:
    ...
    Позволяет переключать текущую стратегию отображения прогресса.
    По умолчанию не отображает прогресс (NoProgressStrategy).
    """
