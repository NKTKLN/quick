"""Модуль построения матрицы коэффициентов для СМО с MAP-потоками и несколькими датчиками.
...
from app.services.matrix_builders.base import BaseMAPMatrixBuilder


class MultiSensorMAPServerMatrixBuilder(BaseMAPMatrixBuilder):
    """Построитель матрицы коэффициентов для СМО с MAP-потоками и несколькими датчиками.
    ...
    def build(self) -> NDArray[np.float64]:
        ...
        logger.success(
            "Матрица коэффициентов для СМО с MAP-потоками и несколькими датчикам "
