...
from quick.services.systems_behavior.map import MAPSystemBehavior
...
class BaseServerSystem(ABC):
    ...
    def calculate_rejection_probability(self) -> NDArray[np.float64]:
        ...
        """
        if self.params.settings.system_type == SystemType.MULTI_SENSOR_MAP:
            if not isinstance(self.system_behavior, MAPSystemBehavior):
                raise TypeError("base_params должен быть MAPSystemBehavior")

            m = self.system_behavior.D0.shape[0]

            p_reject = cast(
                NDArray[np.float64], np.sum(self.probabilities[-m:], axis=0)
            )
            return p_reject
    ...
    def calculate(self) -> dict[str, NDArray[np.float64]]:
        ...
        """
        mode = self.params.settings.calculation_mode

        calculations: dict[CalculationMode, dict[str, Callable]] = {
        ...
        if mode not in calculations:
            raise ValueError(f"Неподдерживаемый режим расчёта: {mode}")

        logger.info(f"Запуск расчёта характеристик СМО в режиме: {mode}")

        result = {
            metric_name: calculation()
            for metric_name, calculation in calculations[mode].items()
        }

        logger.success(f"Расчёт в режиме {mode} успешно завершён")
        return result
