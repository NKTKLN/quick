"""Модуль, содержащий базовый класс для серверных систем массового обслуживания (СМО) с MAP-потоком.

Включает в себя определение класса BaseMAPServerSystem с методами для преобразования
вероятностей состояний и расчёта основных характеристик СМО с MAP-потоком:
...
class BaseMAPServerSystem(BaseServerSystem):
    """Абстрактный базовый класс серверной СМО с MAP-потоком.

    Расширяет базовый класс BaseServerSystem методами, специфичными для систем
    массового обслуживания с MAP-потоком, включая преобразование вероятностей
