...
if submitted:
    ...
    if len(state_variables) > max_customers:
        st.error(
            f"Количество состояний ({len(state_variables)}) не может превышать\
                n = {max_customers}."
        )
        st.stop()

    if len(initial_probabilities) > max_customers:
        st.error(
            f"Количество вероятностей ({len(initial_probabilities)}) не может превышать\
                n = {max_customers}."
        )
        st.stop()
